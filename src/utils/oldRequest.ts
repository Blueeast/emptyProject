...
// const baseURL = process.env.NODE_ENV === 'devlopment' ? MAINHOST : location.origin
const baseURL = MAINHOST
...
// 合并axios参数
const conbineOptions = (Opts: any, data: Datas, method: Methods): AxiosRequestConfig => {
    let opts = Opts
    if (typeof opts === 'string') {
        opts = { url: opts }
    }
    const Data = { ...conmomPrams, ...opts.data, ...data }
    const options = {
    ...
    return options.method !== 'GET' ? Object.assign(options, { data: Data }) :
